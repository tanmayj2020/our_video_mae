...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    """
    def __init__(self, num_classes_classification , img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.head_classification = nn.Linear(embed_dim, num_classes_classification) if num_classes > 0 else nn.Identity()
        if use_learnable_pos_emb:
    ...
    def forward_vanilla(self , x):
        ...
        if self.fc_norm is not None:
            ...
            x= x[:, 0]
        x = self.head_classification(self.fc_dropout(x))
        return x
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask):
        predicted_class = self.encoder.forward_vanilla(x) 
        _, _, T, _, _ = x.shape
